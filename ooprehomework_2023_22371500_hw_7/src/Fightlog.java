...
public class Fightlog {
    ...
    private static HashMap<Adventurer, Integer> fightAdventurerHpMap = new HashMap<>();//体力日志;
    private static ArrayList<Adventurer> fightAdventurerSequence = new ArrayList<>();//顺序记录;
    
    public static void put_into_fightAdventurerMap(int j, String strings) {
        fightAdventurerMap.put(j, strings);
    }
    
    public static void put_into_fightAdventurerHpMap(Adventurer adv, int hp) {
        fightAdventurerHpMap.put(adv, hp);
    }
    
    public static void put_into_fightAdventurerSequence(Adventurer adv) {
        fightAdventurerSequence.add(adv);
    }
    
    public static void clear_fightAdventurerSequence() {
        fightAdventurerSequence.clear();
    }
    
    public static void check_fightAdventurerHpMap() {
        for (Adventurer adventurer : fightAdventurerSequence) { //遍历每一个冒险者，找雇佣关系;
            int beforeHp = fightAdventurerHpMap.get(adventurer);//之前的体力
            int hp = adventurer.getHp();//现在的体力;
            int compare = (int) (0.5 * beforeHp);//进入战斗前的一半;
            if (hp <= compare) { //冒险者的体力不大于进入战斗前的一半;
                long giveMoney = 10000L * (beforeHp - hp);//应该给的钱;
                HashSet<Adventurer> employAdventurerSet = adventurer.getEmployAdventurerSet();
                for (Adventurer employer : employAdventurerSet) { //看它雇佣的人的钱数;
                    if (employer.getMoney() >= giveMoney) { //持有的金钱多于此数值;
                        adventurer.addMoney(giveMoney);
                        employer.subMoney(giveMoney);
                    } else { //没有这么多钱，就把所有的钱给雇主;
                        long actualMoney = employer.getMoney();
                        adventurer.addMoney(actualMoney);
                        employer.subMoney(actualMoney);
                    }
                }
            }
        }
    ...
    public static void clear_fightAdventurerMap() {
        fightAdventurerMap.clear();
    }
    
    public static void clear_fightAdventurerHpMap() {
        fightAdventurerHpMap.clear();
    }
    ...
    public static boolean is_legal_2(Adventurer adv1, Adventurer adv2, String equName) {
        ...
        if (!(fightAdventurerMap.containsValue(n1)) || !(fightAdventurerMap.containsValue(n2))) {
            ...
            if (!adv1.search_equipmentbag(equName)) {
                ...
                Equipment equipment = adv1.get_equipment_bag(equName);
                adv2.attacked(star, level, equipment);
                return true;
            }
        }
        return false;
    ...
    public static boolean is_legal_3(String advName, String equName, Adventurer adventurer) {
        if (!fightAdventurerMap.containsValue(advName)) {
            ...
            if (!adventurer.search_equipmentbag(equName)) {
                ...
                Equipment equipment = adventurer.get_equipment_bag(equName);
                for (int i = 0; i < fightAdventurerMap.size(); i++) {
                    ...
                    if (!advName.equals(name)) { //不是该发起攻击者，成为被攻击者;
                        ...
                        adv.aoe_attacked(star, level, equipment);
                    }
                }
    ...
    public static void add_fightAdventurerMap(int id, String name) {
