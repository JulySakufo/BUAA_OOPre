...
public class Adventurer implements Commodity {
    ...
    private long money;//adventurer拥有的金钱
    ...
    public HashMap<String, ArrayList<Integer>> getBottleBag() {
        return bottleBag;
    }
    
    public HashMap<String, Equipment> getEquipmentBag() {
        return equipmentBag;
    }
    
    public HashMap<String, ArrayList<Integer>> getFoodBag() {
        return foodBag;
    }
    
    public void delete_print(int size, String value) {
        System.out.println(size + " " + value);
    }
    
    public int getBottleMapSize() {
        return bottleMap.size();
    }
    
    public int getEquipmentMapSize() {
        return equipmentMap.size();
    }
    
    public int getFoodMapSize() {
        return foodMap.size();
    }
    
    public void delete_bottle(int botId) {
        Bottle bottle = get_bottle(botId);//根据id的唯一性得到这个具体的药水瓶，然后进行后面的操作；
        long value = bottle.getPrice();//得到想要卖出去药水瓶的price，增加的金钱就为此price;
        sub_price(value);//price更新;
        money = money + value;//卖出去赚钱了，money变多了;
        Shop.bottleRecord(bottle.getCapacity(), value);//对bottle类型进行记录
    ...
    public void delete_equipment(int equId) {
        ...
        sub_price(value);
        money = money + value;
        Shop.equipmentRecord(equipment.getStar(), value);
    ...
    public void delete_food(int foodId) {
        ...
        sub_price(value);
        money = money + value;
        Shop.foodRecord(food.getEnergy(), value);
        ...
        if (arraylist == null) {
            ...
            if (size == 0) {  //假如背包里面未携带该名字的药水瓶;
                ...
                if (bottle instanceof RegularBottle) { //常规瓶子，得到的结果与以前一样;
                    ...
                    if (!regularbottle.getIsEmpty()) { //不为空瓶;
                        hp = hp + capacity;//体力增加;
                        regularbottle.becomeEmpty();//喝了一次，成为空瓶;
                    } else { //为空瓶，扔掉瓶子;
                    ...
                    if (!reinforcedbottle.getIsEmpty()) { //先喝，喝完保留空瓶，下次再喝的时候喝了空瓶然后扔掉空瓶;
                        hp = hp + (int) ((1 + ratio) * capacity); //按照百分比强化药水的计算公式计算;
                        reinforcedbottle.becomeEmpty();
                    } else { //空瓶扔掉瓶子;
                    ...
                    if (!recoverbottle.getIsEmpty()) { //先喝，喝完保留空瓶，下次再喝的时候喝了空瓶然后扔掉空瓶;
                        recoverbottle.becomeEmpty();
    ...
    public long calculate_employer_price(Adventurer adv) { //计算该冒险者拥有的冒险者价值体的总价值;
        ...
        if (map.size() == 0) {
            return adv.getPrice() + adv.getMoney();//这个冒险者只有bot,equip,food,money带来的价值;
        } else { //至于雇主不算money在service里面进行减掉即可,此处为了递归的规整性;
            long value = adv.getPrice() + adv.getMoney();//自身非雇佣的价值,(要算money);
