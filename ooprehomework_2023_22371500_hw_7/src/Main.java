...
public class Main {
    ...
    public static void input(String[] strings, Scanner scanner) {
        ...
        for (int j = 0; j < fightPeople; j++) {
            ...
            Adventurer adventurer = Service.get_adventurer(strings[3 + j]);//找到这个冒险者;
            int hp = adventurer.getHp();
            Fightlog.put_into_fightAdventurerHpMap(adventurer, hp);//记录进入战斗的冒险者的hp;
            Fightlog.put_into_fightAdventurerSequence(adventurer);
        }
        ...
        Fightlog.check_fightAdventurerHpMap();//战斗结束，进行体力金钱的判断操作;
        Fightlog.clear_fightAdventurerMap(); //战斗模式结束，清空这次进入战斗的冒险者，为下一次做准备;
        Fightlog.clear_fightAdventurerHpMap();
        Fightlog.clear_fightAdventurerSequence();
    }
}
